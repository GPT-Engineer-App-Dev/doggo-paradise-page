  ...
  return (
    <div className="min-h-screen bg-green-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-4xl font-bold mb-8 text-center flex items-center justify-center text-green-800">
          <Dog className="mr-2 text-green-600" /> All About Dogs
        </h1>
        
        <section className="bg-white shadow-md rounded-lg p-6 mb-8 border-2 border-green-200">
          <h2 className="text-2xl font-semibold mb-4 text-green-700">Fun Facts About Dogs</h2>
        ...
        <section className="bg-white shadow-md rounded-lg p-6 mb-8 border-2 border-green-200">
          <h2 className="text-2xl font-semibold mb-4 text-green-700">Popular Dog Breeds</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {dogBreeds.map((breed) => (
              <div key={breed.name} className="border border-green-300 rounded p-4 bg-green-50">
                <h3 className="font-semibold text-green-800">{breed.name}</h3>
                <p className="text-green-600">{breed.description}</p>
              </div>
            ))}
          </div>
        </section>

        <section className="bg-white shadow-md rounded-lg p-6 border-2 border-green-200">
          <h2 className="text-2xl font-semibold mb-4 text-green-700">Dog Avenue</h2>
          <div className="aspect-w-16 aspect-h-9 mb-4">
            <img
              src={dogImages[currentImageIndex]}
              alt={`Dog ${currentImageIndex + 1}`}
              className="mx-auto object-cover rounded-lg border-4 border-green-300"
            />
          </div>
          <button
            onClick={nextImage}
